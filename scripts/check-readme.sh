...
need "LG_IMAGE=ghcr.io/viktorsbaikers/looking-glass:v0.1.1" "planned immutable GHCR image/tag"
need 'planned local URL/SHA-256 release metadata' "planned release-asset status"
need 'only after Slice 39 publishes those exact `v0.1.1` assets and verifies the URL/SHA-256' "release publication prerequisite"
need 'The published-image path becomes available only after Slice 39 publishes the planned' "post-publication image pull condition"
